...
class SocialLoginSerializer(serializers.Serializer):
    ...
    def validate(self,data):
        is_real_account = SocialVerifier.checkToken(data['provider'],data['social_token'],data['user_id'])
        print("IS REAL ACCOUNT: ",is_real_account)
        if is_real_account:
            try:
                social_account = SocialAccount.objects.all().get(user_id=data['user_id'],provider=data['provider'])
    ...
    def create(self, validated_data):
        ...
        is_real_account = SocialVerifier.checkToken(user_data['provider'],user_data['social_token'],user_data['user_id'])
        if is_real_account:            
            ...
            user = Account.objects.create_user(user_data['email'],user_data['username'], Account.objects.make_random_password(), **extra_args)
            return user
